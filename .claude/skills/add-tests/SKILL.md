...
3. **Read the target file and its direct dependencies**
    ...
    - Locate the existing test project for this file (pattern: `[Assembly].TestsUnit`) and read any existing test class for this type so the agent does not duplicate coverage.
...
4. **Spawn the c-sharp-qa subagent**
    ...
    > {EXISTING*TEST_CLASS_CONTENT_OR*"None"}
    >
    > **Conventions:**
    >
    > - Test class: `Given[Context]`, sealed, in `[Assembly].TestsUnit`

...
{
    private readonly ISyncScheduler scheduler = Substitute.For<ISyncScheduler>();
    ...
    public void when_constructed_then_localization_service_receives_common_never_key_for_last_sync_text()
    {
        ...
        localizationService.Received(1).GetLocal("Common.Never");
    }
    ...
    public void when_overall_status_text_accessed_while_syncing_then_status_bar_syncing_key_is_used()
    {
        ...
        localizationService.Received(1).GetLocal("StatusBar.Syncing");
    }
    ...
    public void when_overall_status_text_accessed_with_errors_then_status_bar_error_key_is_used()
    {
        ...
        localizationService.Received(1).GetLocal("StatusBar.Error");
    }
    ...
    public void when_overall_status_text_accessed_with_single_conflict_then_status_bar_conflict_key_is_used()
    {
        ...
        localizationService.Received(1).GetLocal("StatusBar.Conflict", Arg.Any<object[]>());
    }
    ...
    public void when_overall_status_text_accessed_with_multiple_conflicts_then_status_bar_conflicts_key_is_used()
    {
        ...
        localizationService.Received(1).GetLocal("StatusBar.Conflicts", Arg.Any<object[]>());
    }
    ...
    public void when_overall_status_text_accessed_with_no_issues_then_dashboard_all_synced_key_is_used()
    {
        ...
        localizationService.Received(1).GetLocal("Dashboard.AllSynced");
    }

    [Fact]
    public void when_add_never_synced_account_then_recalculate_globals_uses_common_never_key_for_last_sync_text()
    {
        localizationService.GetLocal("Common.NeverSynced").Returns("Never synced");
        localizationService.GetLocal("Common.Never").Returns("Never");
        var sut = CreateSut();

        sut.AddAccount(NeverSyncedAccount("acc-1"));

        localizationService.Received().GetLocal("Common.Never");
    }

    [Fact]
    public void when_add_synced_account_then_last_sync_text_equals_synced_accounts_text()
    {
        localizationService.GetLocal("Common.JustNow").Returns("Just now");
        var sut = CreateSut();
    ...
    [Fact]
    public void when_timer_ticks_after_account_added_then_last_sync_key_is_re_evaluated()
    {
        localizationService.GetLocal("Common.JustNow").Returns("Just now");
        var sut = CreateSut();
        sut.AddAccount(RecentlySyncedAccount("acc-1"));
        sut.StartRefreshTimer();
        localizationService.ClearReceivedCalls();

        timer.Tick();

        localizationService.Received().GetLocal("Common.JustNow");
    }
}

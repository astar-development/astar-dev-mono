...
{
    /// <summary>Migrates the database, logging any failure and returning it so the caller can react.</summary>
    ...
    public static Task<Exceptional<UnitFp>> MigrateAsync(IDbContextFactory<ControlDbContext> dbContextFactory, ILogger logger) =>
        Try.RunAsync(() => ApplyPendingMigrationsAsync(dbContextFactory, logger))

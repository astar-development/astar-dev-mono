...
namespace AStar.Dev.OneDriveSync.Logging;
...
public sealed class LoggingService : IDisposable
{
    ...
    private static string GetPlatformLogDirectory()
    {
        if (OperatingSystem.IsLinux())
        {
            ...
            return Path.Combine(basePath, "AStar.Dev.OneDriveSync", "logs");
        }

        if (OperatingSystem.IsMacOS())
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Library", "Logs", "AStar.Dev.OneDriveSync");

        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "AStar.Dev.OneDriveSync", "logs");
    }
}

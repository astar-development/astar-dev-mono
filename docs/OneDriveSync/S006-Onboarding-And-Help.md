...
- [x] First launch (no accounts in DB) routes directly to the onboarding view before the main nav rail is interactive
- [x] Welcome screen explains what the app does in plain language
- [x] Prominent "Add your first account" CTA button visible
- [x] "Skip" option present — routes directly to the Accounts view without completing onboarding (OH-03)

### External Help Link (OH-02)
- [x] Onboarding includes a link that opens an external markdown help file in the system browser
- [x] URL is configurable (not hardcoded) — stored in app config or resource file

### User Type Selection (OH-04, Section 5)
- [x] User type selection (Casual / Power User) presented **before** the add-account wizard CTA during onboarding
- [x] Casual is the default selection
- [x] Switching to Power User during onboarding shows confirmation: "This unlocks advanced settings that can affect sync performance if misconfigured" — user must confirm
- [x] Switching to Casual requires no confirmation
- [x] Selection persisted in SQLite `AppSettings` table
- [x] User type selection also accessible any time in Settings (wired up in S015)

### Help Nav Item (OH-05, OH-06)
- [x] Help icon in nav rail (item 7) opens a view that replays the onboarding content (identical content in MVP)
- [x] Help view accessible at all times from the nav rail — even after onboarding is complete

### Subsequent Launches
- [x] If ≥ 1 account exists in DB, onboarding is **not** shown on launch — app goes directly to Dashboard
- [x] Onboarding can be replayed via the Help nav item at any time

### Tests
- [x] **Unit test**: `OnboardingViewModel` — if no accounts, `ShouldShowOnboarding` is `true`; if accounts exist, `false`
- [x] **Unit test**: User type toggle — Casual → Power User emits confirmation request; Power User → Casual does not
- [x] **Unit test**: Skipping onboarding marks it as complete and routes to Accounts

...
- [x] Three modes supported: **Light**, **Dark**, **Auto**
- [x] "Auto" observes OS theme via Avalonia's `PlatformThemeVariant` or equivalent; responds to OS theme changes in real-time without restart (TH-05)

### Runtime Switching (TH-03)

- [x] Theme change applies **immediately** — resource dictionaries are swapped dynamically; no restart required
- [x] All open views reflect the new theme without navigation

### Hardcoded Colours Forbidden (TH-04)

- [x] Zero hardcoded colour values (`#RRGGBB`, named colours) in any `.axaml` file — all colours reference resource keys
- [x] CI build (or a unit test) statically asserts no hardcoded colours exist in AXAML files

### Persistence (TH-02)

- [x] Theme selection stored in the SQLite `AppSettings` table (single-row settings record)
- [x] Theme loaded and applied before the main window is shown (no flash of wrong theme)

### Extensibility (TH-06)

- [x] Adding a new theme requires only: a new `ResourceDictionary` file + registration in `ThemeService` — no other code changes
- [x] `IThemeService` interface in `Infrastructure/Theming/`; `ThemeService` is the implementation

### Tests

- [x] **Unit test**: `ThemeService.SetTheme(ThemeMode.Dark)` raises a `ThemeChanged` observable/event
- [x] **Unit test**: `ThemeService.SetTheme(ThemeMode.Auto)` subscribes to OS changes (use mock platform provider)
- [x] **Integration test**: theme persists across a simulated restart (write to DB; read back and assert match)

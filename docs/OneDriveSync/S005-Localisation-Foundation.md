...
- [x] All user-facing strings (labels, tooltips, error messages, notification text, log messages where practicable) stored in `.resx` resource files — **no hardcoded strings in AXAML or C# UI code**
- [x] MVP locale: `en-GB` — this is the fallback and only fully translated locale
- [x] `ILocalisationService` interface in `Infrastructure/Localisation/`; `LocalisationService` is the implementation

### Locale Selection (LO-03, LO-04)
- [x] Settings view exposes a locale selector (dropdown); selection applies globally and immediately
- [x] On first launch, OS locale is detected; if not `en-GB` (the only supported locale in MVP), `en-GB` is applied silently
- [x] Selected locale persisted in the SQLite `AppSettings` table

### Date/Time Formatting (LO-07)
- [x] "Last synced" display: relative format for < 1 hour ("5 minutes ago"); absolute format for ≥ 1 hour ("Today at 14:32" / "25 Mar at 09:15")
- [x] All date/time formatting respects the active locale setting
- [x] `IRelativeTimeFormatter` interface + `RelativeTimeFormatter` implementation — testable independently

### Conflict Rename Format (LO-06)
- [x] "Keep Both" conflict rename suffix uses UTC-formatted string: `yyyy-MM-ddTHHmmssZ` — this format is locale-invariant (see CR-04)

### Tests
- [x] **Unit test**: `RelativeTimeFormatter` — values < 1 hour return relative strings; ≥ 1 hour return absolute strings; edge cases at exactly 1 hour
- [x] **Unit test**: conflict rename suffix is locale-invariant (same output regardless of active locale)
- [x] **Integration test**: locale change persists and is restored correctly
- [x] Static analysis / test: no hardcoded UI strings in `*.axaml` or `*ViewModel.cs` files (string literals in UI code fail the test)

...
The system SHALL expose an `IAppBootstrapper` interface with a single `Task BootstrapAsync(IProgress<string> progress, CancellationToken cancellationToken = default)` method. `AppBootstrapper` SHALL implement this interface and receive all startup dependencies via constructor injection.
